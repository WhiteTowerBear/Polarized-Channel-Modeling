...
transmit.len.h=10*par.waveLambda;
transmit.len.v=10*par.waveLambda;
transmit.aperture=sqrt(transmit.len.h^2+transmit.len.v^2);
receive.len.h=10*par.waveLambda;
receive.len.v=10*par.waveLambda;
...
for index_user=1:users.num
    DOF=[];
    source_num_range=[0:30]; % One dimension
    ...
    if index_user==1
        plot(source_num_range.^2,DOF,'ro-','LineWidth',1);
    elseif index_user==2
        plot(source_num_range.^2,DOF,'bx-','LineWidth',1);
    else
...
xlabel('Number of transmit antennas','Interpreter','latex');
    ...
    grid on
legend('UE at $z=5\lambda$','UE at $z=7 \lambda$',...
    'UE at $z=9 \lambda$','Interpreter','Latex');
